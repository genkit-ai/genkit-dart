...
void main() {
  ...
  group('request dialect', () {
    ...
    test('rejects a level xAI does not define', () async {
      // `minimal` and `max` are OpenAI's; the shared options schema offers the
      // union of every provider's vocabulary.
      final ai = Genkit(
      ...
      expect(response.finishReason, FinishReason.failed);
      expect(response.error?.message, contains('minimal'));
    });
