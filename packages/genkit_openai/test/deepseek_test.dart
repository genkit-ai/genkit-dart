...
void main() {
  ...
  group('request dialect', () {
    ...
    test('OpenAI sends no response_format without a schema', () async {
      // Unchanged there: json_schema is the only thing the plugin asks OpenAI
      // for, and it has nothing to build one from.
      final requests = <http.Request>[];
      final ai = Genkit(
        plugins: [openAI(apiKey: 'k', httpClient: recordingClient(requests))],
      );
      ...
      expect(chatBodyOf(requests), isNot(contains('response_format')));
    });

    test('carries the schema in the prompt, since the wire cannot', () async {
      // Genkit's json formatter sets defaultInstructions: false, so nothing
      // else tells the model what to emit - and DeepSeek refuses a JSON
      // request whose prompt never says "json".
      final requests = <http.Request>[];
      final ai = Genkit(
        plugins: [deepSeek(apiKey: 'k', httpClient: recordingClient(requests))],
      );
      ...
      expect((sent['content'] as String).toLowerCase(), contains('json'));
