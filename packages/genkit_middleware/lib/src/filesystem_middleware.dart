...
abstract class $FilesystemOptions {
  @Field(description: 'The root directory to which all filesystem operations are restricted.')
  ...
  List<GenerateMiddlewareDef> middleware() => [
        defineMiddleware<FilesystemOptions>(
          ...
          create: ([FilesystemOptions? config]) {
            if (config == null) {
              throw ArgumentError('filesystem middleware requires a rootDirectory option');
            }
...
class FilesystemMiddleware extends GenerateMiddleware {
        ...
        fn: (input, _) async {
          ...
          Future<List<ListFileOutputItem>> list(String dir, String base) async {
            ...
            await for (final entity in d.list()) {
               ...
               results.add(ListFileOutputItem(
                 path: relativePath,
      ...
      Tool<ReadFileInput, String>(
        ...
        fn: (input, _) async {
          ...
          if (isImage) {
            ...
            parts.add(MediaPart(media: Media(url: uri, contentType: mimeType)));
          } else {
             final content = await file.readAsString();
             parts.add(TextPart(text: '<read_file path="${input.filePath}">\n$content\n</read_file>'));
          }

          if (_messageQueue.isNotEmpty && _messageQueue.last.role == Role.user) {
             final lastMsg = _messageQueue.last;
             _messageQueue.removeLast(); // We will modify and re-add or just modify content
      ...
      Tool<SearchAndReplaceInput, String>(
        ...
        fn: (input, _) async {
          ...
          for (final editBlock in input.edits) {
             ...
             if (!editBlock.startsWith(startMarker) || !editBlock.endsWith(endMarker)) {
               throw Exception('Invalid edit block format. Block must start with "<<<<<<< SEARCH\\n" and end with "\\n>>>>>>> REPLACE"');
             }
             ...
             if (separatorIndices.isEmpty) {
               throw Exception('Invalid edit block format. Missing separator "\\n=======\\n"');
             }
             ...
             for (final splitIndex in separatorIndices) {
                ...
                final replace = innerContent.substring(splitIndex + separator.length);
  ...
  Future<ToolResponse> tool(
    ...
    try {
      ...
      if (['list_files', 'read_file', 'write_file', 'search_and_replace'].contains(request.name)) {
          ...
          if (_messageQueue.isNotEmpty && _messageQueue.last.role == Role.user) {
             ...
             _messageQueue.add(Message(role: Role.user, content: [TextPart(text: errorText)]));
          }
          
          // Return a response to satisfy the signature, but the model will primarily see the user message
          return ToolResponse(name: request.name, output: 'Tool failed. See context for details.');
      }
      rethrow;
    }
  }
