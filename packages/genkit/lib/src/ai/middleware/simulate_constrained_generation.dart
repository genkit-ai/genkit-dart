...
///
/// Matches `jsonFormatter`'s wording byte for byte (`formatters/json.dart`)
/// rather than defining its own, but is not a call into it: that formatter
/// sets `defaultInstructions: false`, so it never injects anything of its
/// own, and this middleware is the whole mechanism regardless of which
/// format asked for the schema. Keeping the wording identical is what lets
/// `_withInstructions` dedupe against a format's own injected instructions by
/// exact text.
...
///
/// `outputNoInstructions` is not honoured here and cannot be: it lives on
/// `GenerateActionOutputConfig.instructions`, which `generate` has already
/// dropped by the time it builds the [OutputConfig] this middleware sees.
/// Threading it through would mean adding a field to a type plugins and the
/// wire format share with JS and Go. JS has the identical gap
/// (`js/ai/src/model/middleware.ts`) for the same reason, so a model that
/// needs simulation gets the schema instructions regardless of the flag,
/// while a model claiming native support still honours it (nothing here
...
/// Returns [messages] with [instructions] appended to the first system message
/// - or the last user message when there is none - or unchanged when they are
/// already present.
///
...
List<Message>? _withInstructions(List<Message> messages, String instructions) {
  // Restricted to system/user: a model or tool turn can echo the rendered
  // schema text (in a transcript, a tool result, ...) without it having come
  // from this middleware, and matching that would suppress injection while
  // still stripping `schema`/`constrained` below - the model would end up
  // with neither the native path nor the instructions.
  ...
  // First system message, last user message, as JS's `injectInstructions`
  // does. The first system message is not cosmetic: a provider that folds the
  // conversation's system messages into one `system` field can keep only the
  // first (genkit_anthropic does), and instructions appended to a later one
  // would be dropped after the schema had already been stripped.
