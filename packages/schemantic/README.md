...
## Bring Your Own Types (BYOT)

Schemantic is designed to be extensible. If you already have data classes defined using other serialization libraries like `json_serializable`, you can easily bridge them into the Schemantic ecosystem using `SchemanticType.from`.

This is particularly useful when using `json_serializable` 6.13.0+, which can automatically generate JSON Schemas.

### Using with `json_serializable`

1. Define your class with `createJsonSchema: true`:
...
2. Use it as a regular `SchemanticType`:
