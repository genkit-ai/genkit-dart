...
final menuRagTool = ai.defineTool(
  ...
  fn: (input, _) async {
    ...
    final results =
        menuItems.where((item) => item.toLowerCase().contains(query)).toList();

    // If no specific matches, return a subset or generic message?
    // The prompt usually handles "if you don't know".
