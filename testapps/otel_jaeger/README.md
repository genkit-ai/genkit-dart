...
Demonstrates `genkit_otel`: OpenTelemetry GenAI semantic-conventions
...
```
dart run (the app) --OTLP:4318--> otelcol-contrib --OTLP:14317--> Jaeger (UI :16686)
...
3. Open Jaeger at http://localhost:16686, pick your service (the default is
   `@dart/dartastic_opentelemetry` unless you set `OTEL_SERVICE_NAME`), and
   inspect the `chat gemini-flash-latest` span (with request config, usage, and
   captured content attributes).
...
```dart
// Zero-config: reads OTEL_* env vars, defaults to http://localhost:4318.
